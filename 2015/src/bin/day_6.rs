...
enum Instruction {
    TurnOn((Coordinates, Coordinates)),
    TurnOff((Coordinates, Coordinates)),
...
impl Instruction {
    pub fn get_coordinates(&self) -> Vec<Coordinates> {
        ...
        let (start, end) = match self {
            Instruction::TurnOn((s, e)) => (s, e),
            Instruction::TurnOff((s, e)) => (s, e),
            Instruction::Toggle((s, e)) => (s, e),
        };
...
fn part1() {
    ...
    let mut turned_on_lights: HashSet<Coordinates> = HashSet::new();

    instructions.into_iter().for_each(|instruction| {
        let coordinates = instruction.get_coordinates();

        match instruction {
            Instruction::TurnOn(_) => {
                for coords in coordinates {
                    turned_on_lights.insert(coords);
                }
            }
            Instruction::TurnOff(_) => {
                for coords in coordinates {
                    turned_on_lights.remove(&coords);
                }
            }
            Instruction::Toggle(_) => {
                for coords in coordinates {
                    if turned_on_lights.contains(&coords) {
                        turned_on_lights.remove(&coords);
                    } else {
                        turned_on_lights.insert(coords);
                    }
    ...
    let len = turned_on_lights.len();

    println!("Part 1: amount of turned on lights: {len}");
}

fn part2() {
    let instructions = parse_instructions(INPUT);

    let mut brightness: HashMap<Coordinates, usize> = HashMap::new();

    instructions.into_iter().for_each(|instruction| {
        let coordinates = instruction.get_coordinates();

        let add: i32 = match instruction {
            Instruction::TurnOn(_) => 1,
            Instruction::TurnOff(_) => -1,
            Instruction::Toggle(_) => 2,
        };

        for coords in coordinates {
            let current = brightness.entry(coords).or_default();
            if (*current as i32 + add) < 0 {
                *current = 0;
            } else {
                *current += add as usize;
            }
        }
    });

    let total_brightness = brightness.into_values().sum::<usize>();
    ...
    input
        ...
        .map(|l| {
            ...
            match (parts[0], parts[1]) {
                ("toggle", _) => {
                    Instruction::Toggle((parse_coordinates(parts[1]), parse_coordinates(parts[3])))
                }

                ("turn", "on") => {
                    Instruction::TurnOn((parse_coordinates(parts[2]), parse_coordinates(parts[4])))
                }
                ("turn", "off") => {
                    Instruction::TurnOff((parse_coordinates(parts[2]), parse_coordinates(parts[4])))
                }
                _ => unreachable!(),
            }
        })
...
fn parse_coordinates(coords: &str) -> Coordinates {
