...
#[derive(Debug)]
struct Matrix {
    nums: Vec<Vec<Option<usize>>>,
}

impl Matrix {
    pub fn new(width: usize, height: usize) -> Self {
        Self {
            nums: vec![vec![None; height]; width],
        }
    }
    ...
    pub fn set(&mut self, x: usize, y: usize, num: usize) {
        ...
        self.nums[x][y] = Some(num)
    }

    pub fn into_column_numbers_right_to_left(self) -> Vec<usize> {
        self.nums
            .into_iter()
            .map(|column| {
                let mut total = 0;
...
#[derive(Debug)]
struct Homework {
    problems: Vec<(Operation, Vec<usize>)>,
}
...
impl Homework {
    pub fn new(input: &str) -> Self {
        ...
        input.lines().for_each(|line| {
            ...
            if parsing_numbers {
                number_vecs.push(parts.iter().map(|p| p.parse::<usize>().unwrap()).collect());
            } else {
                operations = Some(
                    parts
                        .iter()
        ...
        let problems = operations
            ...
            .enumerate()
            .map(|(i, op)| {
                let nums = number_vecs.iter().map(|n| n[i]).collect::<Vec<_>>();
                (op, nums)
            })
...
fn part1() {
    let homework = Homework::new(INPUT);

    let sum = homework
        .problems
        .into_iter()
        .map(|(op, nums)| match op {
            Operation::Multiply => nums.into_iter().reduce(|acc, n| acc * n).unwrap(),
            Operation::Add => nums.into_iter().sum::<usize>(),
        })
        .sum::<usize>();

    println!("Part1: sum of answers: {sum}");
}
